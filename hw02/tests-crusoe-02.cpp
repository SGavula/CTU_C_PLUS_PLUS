    ...
    SECTION("Path in graph 3x3") {
        ...
        for (size_t j = 0; j < 3; ++j) {
            ...
            do {
                ...
                g.is_achievable(i, comp);   
                i++;     
            } while (!comp.empty() && comp.size() <= 1 && i < graph_size * graph_size);
            REQUIRE_FALSE(comp.empty());
            if (i <= graph_size * graph_size) {
                std::vector<size_t> pth = g.path(comp[0], comp[comp.size() - 1]);
                REQUIRE_FALSE(pth.empty());
            }
        }
    }
