...
TEST_CASE("Check mersenne twister generator", "[mersenne][all]") {
    auto check_mt_gen = [] (numbers_generator& mtgen) {
        ...
        CHECK_FALSE(mtgen.random_bool());
        CHECK(mtgen.random_double() == Catch::Approx(0.9457483679));
        CHECK(mtgen.random_double() == Catch::Approx(0.1856478315));
        CHECK(mtgen.random_double() == Catch::Approx(0.9487333737));
...
TEST_CASE("Check minstd generator", "[minstd][all]") {
    auto check_std_gen = [] (numbers_generator& mtgen) {
        ...
        CHECK(mtgen.random_bool());
        CHECK(mtgen.random_bool());
        CHECK(mtgen.random_double() == Catch::Approx(0.4801670224));
        CHECK(mtgen.random_double() == Catch::Approx(0.1423510221));
        CHECK(mtgen.random_double() == Catch::Approx(0.4262081654));
