...
TEST_CASE("Copy", "[stage3]") {
    SECTION("Copy constructor") {
        SECTION("Verify that deep copy is performed 1") {
        ...
        SECTION("Verify that deep copy is performed 2") {
            trie t1({ "a", "aa", "ab", "ad", "cd", "c", "q" });
            trie t2(t1);
            SECTION("Erase from old trie") {
                t1.erase("cd");
                VALIDATE_SETS(extract_all(t1), as_vec({ "a", "aa", "ab", "ad", "c", "q" }));
                VALIDATE_SETS(extract_all(t2), as_vec({ "a", "aa", "ab", "ad", "cd", "c", "q" }));
            }
            SECTION("Erase from new trie") {
                t2.erase("ad");
                VALIDATE_SETS(extract_all(t1), as_vec({ "a", "aa", "ab", "ad", "cd", "c", "q" }));
                VALIDATE_SETS(extract_all(t2), as_vec({ "a", "aa", "ab", "cd", "c", "q" }));
            }
            SECTION("Erase from both tries") {
                t1.erase("cd");
                t2.erase("ad");
                VALIDATE_SETS(extract_all(t1), as_vec({ "a", "aa", "ab", "ad", "c", "q" }));
                VALIDATE_SETS(extract_all(t2), as_vec({ "a", "aa", "ab", "cd", "c", "q" }));
    ...
    SECTION("Move assignment") {
        ...
        SECTION("From non-empty trie") {
            ...
            SECTION("To empty trie") {
                ...
                t2.insert("bag");
                VALIDATE_SETS(extract_all(t2), as_vec({ "ab", "bc", "deg", "bag" }));
            }
            SECTION("To non-empty trie") {
                trie t2({ "kfa", "bfg", "id" });
                t2 = std::move(t1);
    ...
    SECTION("Swapping tries actually swaps elements") {
        ...
        SECTION("Non-empty and non-empty trie") {
            ...
            SECTION("Roundtrip") {
                ...
                t1.erase("azerty");
                t1.insert("eroxy");
                VALIDATE_SETS(extract_all(t1), as_vec({ "eroxy", "qwerty", "qwertz" }));
                t2.erase("awerty");
                t2.insert("erodxy");
                VALIDATE_SETS(extract_all(t2), as_vec({ "qzerty", "querty", "erodxy" }));
            }
        }
    }
}
...
TEST_CASE("Operator==, !=", "[stage3]") {
    ...
    SECTION("Two tries with the same members") {
        ...
        REQUIRE(t1 == t2);
        REQUIRE_FALSE(t1 != t2);
        insert_all(t1, {"aab", "bba", "efgefg", "xkdd"});
        insert_all(t2, {"efgefg", "xkdd", "aab", "bba"});
        REQUIRE(t1 == t2);
        REQUIRE_FALSE(t1 != t2);
    }
    ...
    SECTION("Two lists with different members") {
        ...
        REQUIRE(t1 != t2);
        REQUIRE_FALSE(t1 == t2);
        insert_all(t1, {"bcd", "efg", "xkcd"});
        insert_all(t2, {"efgh", "bc", "xkcd", "bcd"});
        REQUIRE(t1 == t2);
        REQUIRE_FALSE(t1 != t2);
    }
}
